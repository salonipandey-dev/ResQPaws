...
export default function ReportPage() {
  ...
  const handle = (e: any) => { e.preventDefault(); toast.success("Report submitted! NGO #PawCare assigned."); };
  return (
    <form onSubmit={handle} className="max-w-3xl space-y-6">
      <div className="rounded-2xl border border-border bg-card p-6">
        <p className="font-semibold">1. Photo of the animal</p>
        <p className="text-xs text-muted-foreground">Our AI auto-detects species and severity.</p>
      ...
      <div className="rounded-2xl border border-border bg-card p-6 space-y-4">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div><Label>Species</Label>
            <Select><SelectTrigger className="mt-1.5"><SelectValue placeholder="Auto-detected" /></SelectTrigger>
              <SelectContent>{["Dog","Cat","Cow","Bird","Other"].map(s => <SelectItem key={s} value={s}>{s}</SelectItem>)}</SelectContent>
            </Select></div>
          <div><Label>Severity</Label>
            <Select><SelectTrigger className="mt-1.5"><SelectValue placeholder="Auto-scored" /></SelectTrigger>
              <SelectContent>{["Critical","Serious","Stable","Minor"].map(s => <SelectItem key={s} value={s}>{s}</SelectItem>)}</SelectContent>
            </Select></div>
        </div>
        <div><Label>Location</Label><div className="relative"><MapPin className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" /><Input className="pl-9 mt-1.5" placeholder="Detecting your location…" defaultValue="Bandra West, Mumbai" /></div></div>
        <div><Label>Description</Label><Textarea className="mt-1.5 min-h-[100px]" placeholder="Briefly describe the situation…" /></div>
      </div>
