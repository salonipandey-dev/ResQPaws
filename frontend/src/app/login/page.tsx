...
export default function LoginPage() {
  ...
  const onSubmit = (e: React.FormEvent, mode: string) => {
    e.preventDefault();
    const r = ROLES.find(r => r.id === role)!;
    toast.success(`${mode} successful as ${r.label}!`);
    router.push(r.route);
  };
            ...
            <Tabs defaultValue="login" className="mt-6">
              ...
              <TabsContent value="login" className="mt-5">
                <form onSubmit={(e) => onSubmit(e, "Login")} className="space-y-4">
                  <div><Label>Email</Label><Input type="email" placeholder="you@email.com" required className="mt-1.5" /></div>
                  <div><Label>Password</Label><Input type="password" placeholder="••••••••" required className="mt-1.5" /></div>
                  ...
                  <Button type="submit" className="w-full rounded-full h-11 shadow-glow">Sign in</Button>
                </form>
              </TabsContent>
              <TabsContent value="register" className="mt-5">
                <form onSubmit={(e) => onSubmit(e, "Register")} className="space-y-4">
                  <div><Label>Full name</Label><Input placeholder="Jane Doe" required className="mt-1.5" /></div>
                  <div><Label>Email</Label><Input type="email" placeholder="you@email.com" required className="mt-1.5" /></div>
                  <div><Label>Password</Label><Input type="password" placeholder="min 8 chars" required className="mt-1.5" /></div>
                  <Button type="submit" className="w-full rounded-full h-11 shadow-glow">Create account</Button>
                </form>
              </TabsContent>
            </Tabs>
            <p className="mt-6 text-xs text-center text-muted-foreground">Protected by industry-grade encryption • GDPR compliant</p>
          </motion.div>
        </div>
      </div>
